...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        site_filtered_df = site_df['class'].value_counts().reset_index()
        site_filtered_df.columns = ['class', 'count']

        # More meaningful labels (Success or Failure)
        site_filtered_df['class'] = site_filtered_df['class'].map({0: 'Failure', 1: 'Success'})

        fig = px.pie(site_filtered_df, values='count', 
        names='class', 
...
def get_payload_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        site_filter_df = filtered_payload_df[filtered_payload_df['Launch Site'] == entered_site]
        fig = px.scatter(site_filter_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig
